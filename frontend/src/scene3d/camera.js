/**
 * scene3d/camera.js
 * ─────────────────
 * ArcRotateCamera 控制 + 过渡动画（聚焦景点、路线包围盒、入场动画等）。
 * 对外暴露纯坐标/数值接口，不泄露 Babylon 对象。
 */

let DEFAULT_ALPHA = -Math.PI / 2;
let DEFAULT_BETA = Math.PI / 4; // ~45° 俯角
let DEFAULT_RADIUS = 120;
const DEFAULT_TARGET = [0, 0, 0];

/**
 * @param {BABYLON.Scene} scene
 * @param {HTMLCanvasElement} canvas
 * @param {object} [opts] 可选覆盖默认相机参数
 */
export function createCameraRig(scene, canvas, opts = {}) {
  const BABYLON = window.BABYLON;

  // 如果传入了 target，同步更新默认值（resetToDefault 时用）
  if (opts.target && Array.isArray(opts.target) && opts.target.length >= 3) {
    DEFAULT_TARGET[0] = opts.target[0];
    DEFAULT_TARGET[1] = opts.target[1];
    DEFAULT_TARGET[2] = opts.target[2];
  }

  const target = new BABYLON.Vector3(...DEFAULT_TARGET);

  // 从 position + target 推算 ArcRotateCamera 的 alpha/beta/radius
  let initAlpha = opts.alpha ?? DEFAULT_ALPHA;
  let initBeta = opts.beta ?? DEFAULT_BETA;
  let initRadius = opts.radius ?? DEFAULT_RADIUS;

  if (opts.position && Array.isArray(opts.position) && opts.position.length >= 3) {
    const pos = new BABYLON.Vector3(opts.position[0], opts.position[1], opts.position[2]);
    const diff = pos.subtract(target);
    initRadius = diff.length();
    // alpha = atan2(x, z) 在 Babylon ArcRotate 中是绕 Y 轴角度
    initAlpha = Math.atan2(diff.x, diff.z);
    // beta = 从 Y 轴向下的角度
    initBeta = Math.acos(Math.max(-1, Math.min(1, diff.y / initRadius)));
  }

  // 同步默认值，供 resetToDefault 使用
  DEFAULT_ALPHA = initAlpha;
  DEFAULT_BETA = initBeta;
  DEFAULT_RADIUS = initRadius;

  const camera = new BABYLON.ArcRotateCamera(
    "mainCamera",
    initAlpha,
    initBeta,
    initRadius,
    target,
    scene
  );

  camera.attachControl(canvas, true);
  camera.lowerRadiusLimit = 5;
  camera.upperRadiusLimit = 500;
  camera.upperBetaLimit = Math.PI * 0.495; // 不让翻到底下
  camera.minZ = 0.5;
  camera.maxZ = 20000; // 确保能看到 skybox（size=10000）
  camera.panningSensibility = 200;
  camera.wheelDeltaPercentage = 0.02;
  camera.pinchDeltaPercentage = 0.01;
  // 移动端触控：单指旋转，双指缩放+平移
  camera.inputs.attached.pointers.multiTouchPanAndZoom = true;

  // 惯性阻尼
  camera.inertia = 0.9;
  camera.panningInertia = 0.85;

  // ─── 相机高度限制：防止相机移到地面以下 ───
  const MIN_TARGET_Y = 0;       // target 不低于地面
  const MIN_CAMERA_Y = 0.5;     // 相机位置最低高度
  camera.onAfterCheckInputsObservable.add(() => {
    // 限制 target.y（平移时可能拖到地下）
    if (camera.target.y < MIN_TARGET_Y) {
      camera.target.y = MIN_TARGET_Y;
    }
    // 安全兜底：确保相机实际位置不低于最小高度
    if (camera.position.y < MIN_CAMERA_Y) {
      camera.position.y = MIN_CAMERA_Y;
    }
  });

  // ─── 动画状态 ───
  let _anim = null; // { startTime, duration, from, to, onComplete }

  /**
   * 入场动画：绕目标点旋转 360° + 拉近。
   * @param {{ duration?: number }} [animOpts]
   * @returns {Promise<void>}
   */
  function playIntro(animOpts = {}) {
    const duration = animOpts.duration || 3000;
    const startAlpha = camera.alpha;
    const endAlpha = startAlpha + Math.PI * 2;
    const startRadius = camera.radius * 2.5;
    const endRadius = camera.radius;

    camera.radius = startRadius;

    return new Promise((resolve) => {
      camera.detachControl();
      _anim = {
        startTime: performance.now(),
        duration,
        update(t) {
          const k = easeOutQuart(t);
          camera.alpha = startAlpha + (endAlpha - startAlpha) * k;
          camera.radius = startRadius + (endRadius - startRadius) * k;
        },
        onComplete: () => {
          camera.attachControl(canvas, true);
          resolve();
        },
      };
    });
  }

  /**
   * 聚焦到世界坐标点。
   * @param {{ x: number, y: number, z: number }} point
   * @param {{ radius?: number, duration?: number }} [animOpts]
   */
  function focusOnPoint(point, animOpts = {}) {
    const duration = animOpts.duration || 1000;
    const endRadius = animOpts.radius ?? 40;

    // 与地面成 30 度角 → 从天顶算 60 度
    const endBeta = Math.PI / 3;

    // ★ 把 target 向下偏移，让模型出现在画面上方约 40% 位置
    // 偏移量 = radius * sin(beta) * 偏移比例
    // 屏幕中心是 50%，想让模型在 40%，需要往下偏 10% 的视野
    const targetOffsetY = -endRadius * Math.sin(endBeta) * 0.25;
    const endTarget = new BABYLON.Vector3(
      point.x,
      point.y + targetOffsetY,
      point.z
    );

    const startTarget = camera.target.clone();
    const startRadius = camera.radius;
    const startAlpha = camera.alpha;
    const startBeta = camera.beta;

    camera.detachControl();
    _anim = {
      startTime: performance.now(),
      duration,
      update(t) {
        const k = easeInOutCubic(t);
        BABYLON.Vector3.LerpToRef(startTarget, endTarget, k, camera.target);
        camera.radius = startRadius + (endRadius - startRadius) * k;
        camera.beta = startBeta + (endBeta - startBeta) * k;
      },
      onComplete: () => camera.attachControl(canvas, true),
    };
  }

  /**
   * 聚焦包住一组点（路线/景点列表）。
   * @param {Array<{ x: number, y?: number, z: number }>} points
   * @param {{ duration?: number }} [animOpts]
   */
  function focusOnBounds(points, animOpts = {}) {
    if (!points?.length) return;
    const BABYLON_NS = window.BABYLON;
    let minX = Infinity, maxX = -Infinity, minZ = Infinity, maxZ = -Infinity;
    for (const p of points) {
      if (p.x < minX) minX = p.x;
      if (p.x > maxX) maxX = p.x;
      if (p.z < minZ) minZ = p.z;
      if (p.z > maxZ) maxZ = p.z;
    }
    const cx = (minX + maxX) / 2;
    const cz = (minZ + maxZ) / 2;
    const span = Math.max(maxX - minX, maxZ - minZ, 16);
    const radius = Math.min(Math.max(span * 1.5, 35), 220);
    focusOnPoint({ x: cx, y: 0, z: cz }, { radius, duration: animOpts.duration || 1100 });
  }

  /**
   * 正上方俯视（LBS 定位居中）。
   * @param {{ x: number, z: number }} point
   * @param {{ height?: number }} [opts]
   */
  function focusTopDown(point, opts = {}) {
    const height = opts.height ?? 60;
    const endTarget = new BABYLON.Vector3(point.x, 0, point.z);
    const startTarget = camera.target.clone();
    const startBeta = camera.beta;
    const startRadius = camera.radius;

    camera.detachControl();
    _anim = {
      startTime: performance.now(),
      duration: 1100,
      update(t) {
        const k = easeInOutCubic(t);
        BABYLON.Vector3.LerpToRef(startTarget, endTarget, k, camera.target);
        camera.beta = startBeta + (0.01 - startBeta) * k; // 几乎正上方
        camera.radius = startRadius + (height - startRadius) * k;
      },
      onComplete: () => camera.attachControl(canvas, true),
    };
  }

  /**
   * 回到默认视角（精确恢复到初始 position + target）。
   */
  function resetToDefault() {
    const duration = 1100;
    const endTarget = new BABYLON.Vector3(DEFAULT_TARGET[0], DEFAULT_TARGET[1], DEFAULT_TARGET[2]);
    const endAlpha = DEFAULT_ALPHA;
    const endBeta = DEFAULT_BETA;
    const endRadius = DEFAULT_RADIUS;

    const startTarget = camera.target.clone();
    const startAlpha = camera.alpha;
    const startBeta = camera.beta;
    const startRadius = camera.radius;

    camera.detachControl();
    _anim = {
      startTime: performance.now(),
      duration,
      update(t) {
        const k = easeInOutCubic(t);
        BABYLON.Vector3.LerpToRef(startTarget, endTarget, k, camera.target);
        camera.alpha = startAlpha + (endAlpha - startAlpha) * k;
        camera.beta = startBeta + (endBeta - startBeta) * k;
        camera.radius = startRadius + (endRadius - startRadius) * k;
      },
      onComplete: () => camera.attachControl(canvas, true),
    };
  }

  /**
   * 每帧更新（由 SceneManager 的 render loop 调用）。
   */
  function update() {
    if (!_anim) return;
    const elapsed = performance.now() - _anim.startTime;
    const t = Math.min(1, elapsed / _anim.duration);
    _anim.update(t);
    if (t >= 1) {
      const cb = _anim.onComplete;
      _anim = null;
      if (cb) cb();
    }
  }

  /**
   * 飞到任意相机位置 + 目标点（平滑动画过渡）。
   * @param {{ x: number, y: number, z: number }} position
   * @param {{ x: number, y: number, z: number }} targetPt
   * @param {{ duration?: number }} [animOpts]
   */
  function flyToView(position, targetPt, animOpts = {}) {
    const duration = animOpts.duration || 1100;
    const endTarget = new BABYLON.Vector3(targetPt.x, targetPt.y, targetPt.z);
    const endPos = new BABYLON.Vector3(position.x, position.y, position.z);
    const diff = endPos.subtract(endTarget);
    const endRadius = diff.length();
    const endAlpha = Math.atan2(diff.x, diff.z);
    const endBeta = Math.acos(Math.max(-1, Math.min(1, diff.y / endRadius)));

    const startTarget = camera.target.clone();
    const startAlpha = camera.alpha;
    const startBeta = camera.beta;
    const startRadius = camera.radius;

    camera.detachControl();
    _anim = {
      startTime: performance.now(),
      duration,
      update(t) {
        const k = easeInOutCubic(t);
        BABYLON.Vector3.LerpToRef(startTarget, endTarget, k, camera.target);
        camera.alpha = startAlpha + (endAlpha - startAlpha) * k;
        camera.beta = startBeta + (endBeta - startBeta) * k;
        camera.radius = startRadius + (endRadius - startRadius) * k;
      },
      onComplete: () => camera.attachControl(canvas, true),
    };
  }

  /**
   * 聚焦到景点，确保模型和上方排队时间标签都完整可见。
   * @param {{ x: number, y: number, z: number }} position 模型中心位置
   * @param {number} labelTopY 标签顶部的世界坐标 Y 值
   * @param {{ duration?: number }} [animOpts]
   */
  function focusOnAttractionWithLabel(position, labelTopY, animOpts = {}) {
    const duration = animOpts.duration || 900;

    // 需要确保从模型底部（position.y）到标签顶部（labelTopY）都在视野内
    const modelBottomY = Math.max(position.y - 1, 0);
    const totalHeight = labelTopY - modelBottomY;

    // target 瞄准模型与标签之间偏下 40% 的位置，让模型占据画面中下部
    const targetY = modelBottomY + totalHeight * 0.4;
    const endTarget = new BABYLON.Vector3(position.x, targetY, position.z);

    // beta: 倾斜俯视角，约 55°（0.95 弧度），兼顾看到模型全貌和上方标签
    const endBeta = 0.95;

    // radius: 根据需要覆盖的垂直范围计算，确保标签不被裁剪
    // 垂直视角约 camera.fov/2，可见垂直范围 ≈ 2 * radius * sin(beta) * tan(fov/2)
    // 简化：radius ≈ totalHeight / (sin(beta) * 0.8)
    const minRadius = 28;
    const computedRadius = Math.max(totalHeight / (Math.sin(endBeta) * 0.7), minRadius);
    const endRadius = Math.min(computedRadius, 120); // 上限防止太远

    const startTarget = camera.target.clone();
    const startRadius = camera.radius;
    const startBeta = camera.beta;
    const startAlpha = camera.alpha;

    // 保持当前 alpha（水平旋转角），不改变观看方向
    const endAlpha = startAlpha;

    camera.detachControl();
    _anim = {
      startTime: performance.now(),
      duration,
      update(t) {
        const k = easeInOutCubic(t);
        BABYLON.Vector3.LerpToRef(startTarget, endTarget, k, camera.target);
        camera.alpha = startAlpha + (endAlpha - startAlpha) * k;
        camera.beta = startBeta + (endBeta - startBeta) * k;
        camera.radius = startRadius + (endRadius - startRadius) * k;
      },
      onComplete: () => camera.attachControl(canvas, true),
    };
  }

  /**
   * 自动调整相机视角，使起点和终点都在画面中可见（导航启动时调用）。
   * @param {{ x: number, z: number }} start 起点（用户位置）
   * @param {{ x: number, z: number }} end   终点（目标景点）
   * @param {{ duration?: number, padding?: number }} [animOpts]
   */
  function fitRouteEndpoints(start, end, animOpts = {}) {
    const duration = animOpts.duration || 1200;
    const basePadding = animOpts.padding ?? 1.5;

    // ─── aspect ratio 适配：竖屏时需要更大的 padding ───
    const engine = camera.getEngine();
    const canvasW = engine.getRenderWidth();
    const canvasH = engine.getRenderHeight();
    const aspectRatio = canvasW / canvasH;
    // 横屏（aspect >= 1）保持 basePadding 不变；竖屏按比例放大
    const padding = basePadding / Math.min(1, aspectRatio);

    // 中点作为 target
    const cx = (start.x + end.x) / 2;
    const cz = (start.z + end.z) / 2;

    // 两点距离
    const dx = end.x - start.x;
    const dz = end.z - start.z;
    const dist = Math.sqrt(dx * dx + dz * dz);

    // 俯视角度：较高的俯视角让两个点更容易同时出现在画面中
    const endBeta = 0.65; // ~37° from top，略微抬起视角避免端点被压缩出画面

    // radius 根据距离计算：确保两点在视野内
    // 考虑 beta 角度下可见范围 ≈ radius * sin(beta)
    // 竖屏且横向分布时额外补偿：窄屏横向可视范围更小
    let effectiveDist = dist;
    if (aspectRatio < 1 && Math.abs(dx) > Math.abs(dz)) {
      // 两点主要沿横向展开，在窄屏中水平视野受限，额外放大
      effectiveDist = dist * (1 / aspectRatio) * 0.85;
    }
    const minRadius = 60;
    const computedRadius = Math.max(effectiveDist * padding / (2 * Math.sin(endBeta)), minRadius);
    const endRadius = Math.min(computedRadius, 150); // 上限防止拉太远

    const endTarget = new BABYLON.Vector3(cx, 0, cz);

    const startTarget = camera.target.clone();
    const startRadius = camera.radius;
    const startBeta = camera.beta;
    const startAlpha = camera.alpha;

    // alpha 朝向：让相机从两点连线的侧面观察（垂直于连线方向）
    // 这样两点横向展开，更容易同时看到
    const lineAngle = Math.atan2(dx, dz);
    const endAlpha = lineAngle + Math.PI * 3 / 4;

    camera.detachControl();
    _anim = {
      startTime: performance.now(),
      duration,
      update(t) {
        const k = easeInOutCubic(t);
        BABYLON.Vector3.LerpToRef(startTarget, endTarget, k, camera.target);
        camera.alpha = startAlpha + (endAlpha - startAlpha) * k;
        camera.beta = startBeta + (endBeta - startBeta) * k;
        camera.radius = startRadius + (endRadius - startRadius) * k;
      },
      onComplete: () => camera.attachControl(canvas, true),
    };
  }

  return {
    camera,
    playIntro,
    focusOnPoint,
    focusOnBounds,
    focusTopDown,
    flyToView,
    fitRouteEndpoints,
    focusOnAttractionWithLabel,
    resetToDefault,
    update,
    /** 供外部设置默认目标（从 config 传入） */
    setDefaultTarget(x, y, z) {
      DEFAULT_TARGET[0] = x;
      DEFAULT_TARGET[1] = y;
      DEFAULT_TARGET[2] = z;
    },
  };
}

// ─── 缓动函数 ───
function easeOutQuart(t) {
  return 1 - (1 - t) ** 4;
}
function easeInOutCubic(t) {
  return t < 0.5 ? 4 * t * t * t : 1 - (-2 * t + 2) ** 3 / 2;
}
